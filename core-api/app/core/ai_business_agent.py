...
def _structured_protocol_prompt(
    ...
    *,
    ...
    return (
        ...
        "Each business.query call must be a single simple SELECT reading exactly ONE ai_* view. "
        "JOIN, UNION, INTERSECT, EXCEPT, CTEs (WITH ...), and nested SELECTs are never allowed and will always "
        "be rejected — do not attempt them, including across multiple attempts.\n"
        "To answer a request that spans several business domains (e.g. a full business overview/portrait), "
        "issue several SEPARATE business.query calls, one per relevant ai_* view, across successive rounds, "
        "then combine and cross-reference the results yourself in the final answer. Never try to combine "
        "tables inside SQL.\n"
