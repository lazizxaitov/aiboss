...
from app.core.ai_routing import (
    AIRoutingConfig,
    AIRoutingResponse,
    AITaskRouter,
...
async def get_ai_routing(store: Annotated[CoreDataStore, Depends(get_core_store)]) -> AIRoutingResponse:
    ...
    response = await asyncio.to_thread(get_routing_response, store)
...
async def update_ai_routing(
    ...
    try:
        ...
        await asyncio.to_thread(AITaskRouter(store).save_config, config)
    except ValueError as error:
        raise HTTPException(status_code=422, detail=str(error)) from error
    return await asyncio.to_thread(get_routing_response, store)
