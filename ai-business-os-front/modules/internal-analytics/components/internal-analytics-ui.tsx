...
export function AnalyticsTrendChart({
  ...
  return (
    <div className="rounded-[28px] border border-[#3a3d43] bg-[linear-gradient(180deg,#2E3137_0%,#26292e_100%)] p-4 sm:p-5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {comparePrevious ? (
          <span className="inline-flex items-center gap-2 rounded-full border border-[#3a3d43] bg-[#2E3137] px-3 py-1.5 text-xs font-medium text-slate-400">
...
export function SelectField({ label, value, options, onChange, compact }: SelectFieldProps) {
  return (
    <FilterSelect
      ...
      onChange={onChange}
      placeholder="Все"
      className={cn("min-w-0", compact ? "text-xs" : "text-sm")}
      panelClassName="w-[min(320px,calc(100vw-1rem))]"
    />
  );
}
...
export function SearchField({
  ...
  return (
    <label className={cn("flex min-w-0 flex-col gap-2", compact ? "text-xs" : "text-sm")}>
      <span className="text-[11px] uppercase tracking-[0.28em] text-slate-400">Поиск</span>
      <input
        value={value}
        onChange={(event) => onChange(event.target.value)}
        placeholder={placeholder}
        className="w-full rounded-2xl border border-[#3a3d43] bg-[#2E3137] px-4 py-3 text-sm text-slate-200 outline-none transition hover:border-[#4a4e56] focus:border-yellow-300 focus:ring-2 focus:ring-yellow-100"
      />
    </label>
  );
}
...
export function MetricCard({ label, value, note, formula, change, active, onClick }: MetricCardProps) {
  return (
    <button type="button" title={formula} onClick={onClick} className={cn("group text-left", onClick ? "cursor-pointer" : "cursor-default")}>
      <Surface className={cn("h-full p-4 transition", active ? "ring-2 ring-yellow-200" : "hover:border-[#4a4e56]")}>
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full border-separate border-spacing-0">
        <thead className="sticky top-0 z-10 bg-[#2E3137]">
          <tr>
            {columns.map((column) => (
              <th key={column.key} className="border-b border-[#3a3d43] px-4 py-3 text-left text-xs uppercase tracking-[0.24em] text-slate-400">
                <button type="button" onClick={() => onSort(column.key)} className="inline-flex items-center gap-2 text-left">
                  <span>{column.label}</span>
                  {sortKey === column.key ? <span className="text-[10px] text-yellow-300">{sortDirection === "asc" ? "▲" : "▼"}</span> : null}
                </button>
              </th>
            ))}
          </tr>
...
export function EmptyState({ text }: { text: string }) {
  return <div className="rounded-2xl border border-dashed border-[#3a3d43] bg-[#343840] px-4 py-5 text-sm text-slate-400">{text}</div>;
}
